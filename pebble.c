...
void pebbling_strategy(DAG *g,unsigned int upper_bound) {
  ...
  */
  ...
  /* Wall time counter */
  Counter stats_walltime=0;
  /* Counters for interval amounts */
  Counter stats_processed=0;              /* Element processed */
  Counter stats_queued=1;                 /* Element inserted in the queue */
  Counter stats_queued_and_discarded=0;   /* Element inserted in the queue */
  Counter stats_first_queuing=1;          /* Queued elements encountered for the first time */
  Counter stats_requeuing=0;              /* Old elements pushed back in the queue */
  Counter stats_above_upper_bound=0;      /* Element above the upper bounds */
  Counter stats_suboptimal=0;             /* Element discarded because better path exists */
  Counter stats_offspring=0;              /* Element generated by a processed element */
  /* Counters for totals */
  Counter stats_processed_T=0;              /* Element processed */
  Counter stats_queued_T=0;                 /* Element inserted in the queue */
  Counter stats_queued_and_discarded_T=0;   /* Element inserted in the queue */
  Counter stats_first_queuing_T=0;          /* Queued elements encountered for the first time */
  Counter stats_requeuing_T=0;              /* Old elements pushed back in the queue */
  Counter stats_above_upper_bound_T=0;      /* Element above the upper bounds */
  Counter stats_suboptimal_T=0;             /* Element discarded because better path exists */
  Counter stats_offspring_T=0;              /* Element generated by a processed element */

  Counter stats_confs_upper_bound=0;      /* A upper bound on uniques configurations */

  int p,k;
  Counter fixed_size_confs;
  for(p=0;p<=upper_bound;p++) {
    ...
    stats_confs_upper_bound += 2*fixed_size_confs; /* Sink maybe touched or not */
  }
#endif
  ...
  for(resetSL(Q);!isemptySL(Q);delete_and_nextSL(Q)) {
    ...
    if (print_running_stats_flag) {
      stats_walltime+=REPORT_INTERVAL;
      /* Compute statistics from the heuristic */
      stats_processed_T            += stats_processed;
      stats_queued_T               += stats_queued;
      stats_queued_and_discarded_T += stats_queued_and_discarded;
      stats_first_queuing_T        += stats_first_queuing;
      stats_requeuing_T            += stats_requeuing;
      stats_above_upper_bound_T    += stats_above_upper_bound;
      stats_suboptimal_T           += stats_suboptimal;
      stats_offspring_T            += stats_offspring;
      ...
      fprintf(stderr,"[   Unique] = %llu of %llu\n",stats_first_queuing_T,
              stats_confs_upper_bound);
      fprintf(stderr,"[ Requeued] = %llu\n\n",stats_requeuing_T);
      fprintf(stderr,"[Processed] = %llu\n",stats_processed_T);
      fprintf(stderr,"[Discarded] = %llu\n\n",stats_queued_and_discarded_T);
      fprintf(stderr,"[Offspring] = %llu\n",stats_offspring_T);
      fprintf(stderr,"[Sub Optim] = %llu\n",stats_suboptimal_T);
      fprintf(stderr,"[Expensive] = %llu\n",stats_above_upper_bound_T);
      fprintf(stderr,"[Avg.Neig.] = %llu\n",(stats_offspring_T+1)/(stats_processed_T+1));
      ...
      fprintf(stderr,"[ Requeued] = %llu\n\n",stats_requeuing);
      fprintf(stderr,"[Processed] = %llu\n",stats_processed);
      fprintf(stderr,"[Discarded] = %llu\n\n",stats_queued_and_discarded);
      fprintf(stderr,"[Offspring] = %llu\n",stats_offspring);
      fprintf(stderr,"[Sub Optim] = %llu\n",stats_suboptimal);
      fprintf(stderr,"[Expensive] = %llu\n",stats_above_upper_bound);


      fprintf(stderr,"\nPER SECOND\n");
      fprintf(stderr,"[   Queued] = %llu\n",stats_queued_T/stats_walltime);
      fprintf(stderr,"[   Unique] = %llu\n",stats_first_queuing_T/stats_walltime);
      fprintf(stderr,"[ Requeued] = %llu\n\n",stats_requeuing_T/stats_walltime);
      fprintf(stderr,"[Processed] = %llu\n",stats_processed_T/stats_walltime);
      fprintf(stderr,"[Discarded] = %llu\n\n",stats_queued_and_discarded_T/stats_walltime);
      fprintf(stderr,"[Offspring] = %llu\n",stats_offspring_T/stats_walltime);
      fprintf(stderr,"[Sub Optim] = %llu\n",stats_suboptimal_T/stats_walltime);
      fprintf(stderr,"[Expensive] = %llu\n",stats_above_upper_bound_T/stats_walltime);

      /* Reset of intermediate counters */
      stats_processed=0;
      stats_queued=0;
      stats_queued_and_discarded=0;
      stats_first_queuing=0;
      stats_requeuing=0;
      stats_above_upper_bound=0;
      stats_suboptimal=0;
      stats_offspring=0;
...
#ifdef PRINT_RUNNING_STATS
        stats_queued +=1 ;
        if (res.value!=NULL) stats_requeuing +=1;
        else stats_first_queuing +=1;
#endif
